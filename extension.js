...
function activate(context) {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('arduino-cli.createSketchYaml', commandCreateSketchYaml),
 ...
 */
async function commandCreateSketchYaml() {
  if (!(await ensureCliReady())) return;
  const ino = await pickInoFromWorkspace();
  if (!ino) return;
  const sketchDir = path.dirname(ino);
  ...
  const channel = getOutput();
  channel.show(true);
  channel.appendLine(t('sketchYamlCreateStart', { dir: sketchDir }));
  const exists = await pathExists(yamlUri);
  if (exists) {
    const act = await vscode.window.showInformationMessage(t('sketchYamlExistsOpen'), t('open'), t('cancel'));
    if (act === t('open')) {
      vscode.window.showTextDocument(yamlUri);
    }
    channel.appendLine(t('sketchYamlExists', { path: yamlUri.fsPath }));
    return;
  }
  const fqbn = extContext?.workspaceState.get(STATE_FQBN, '');
  let content = buildSketchYamlContent();
  // Try to append profiles from --dump-profile
  if (!fqbn) channel.appendLine(t('sketchYamlNoFqbn')); else channel.appendLine(t('sketchYamlFetching'));
  const profileYaml = fqbn ? await getDumpProfileYaml(fqbn, sketchDir) : '';
  if (profileYaml) {
    content += '# Profiles generated by: arduino-cli compile --fqbn ' + fqbn + ' --dump-profile' + '\n';
    content += '# Docs: https://arduino.github.io/arduino-cli/latest/sketch-project-file/' + '\n';
    content += profileYaml;
    const profileName = extractProfileNameFromDump(profileYaml, fqbn);
    if (profileName) {
      content += `\ndefault_profile: ${encodeYamlString(profileName)}\n`;
      channel.appendLine(t('defaultProfileSet', { name: profileName }));
    }
  } else {
    channel.appendLine(t('sketchYamlEmpty'));
  }
  await writeTextFile(yamlUri, content);
  await vscode.window.showTextDocument(yamlUri);
  vscode.window.showInformationMessage(t('sketchYamlCreated'));
  channel.appendLine(t('sketchYamlCreateDone', { path: yamlUri.fsPath }));
}

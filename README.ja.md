...
- **Debug Sketch** – プロファイルにひも付いたデバッガー設定を自動で読み込み、Cortex-Debug があれば `cortex-debug` を起動し、ない場合は Microsoft C/C++ デバッガーにフォールバックします。その際は `request: "launch"` を自動指定するため、「アタッチするプロセスを選択してください」というダイアログはもう表示されません。
...
2. **「Arduino CLI: Debug Sketch」を実行する。** 対象スケッチとプロファイルを選ぶと `.vscode/` 配下に 2 つの成果物が生成されます。
  ...
  - `launch.json` には Cortex-Debug 用の構成（拡張機能が入っている場合）と Microsoft C/C++ 用のフォールバックが追加されます。必要な OpenOCD / GDB 設定は Arduino CLI の出力から自動取得されるため、手作業で引数を調整する必要はありません。
  プロファイルごとに初回は必ずこのコマンドを実行して構成ファイルを生成してください。コアやボードパッケージ、ツールのバージョンを更新した際も再実行すると、Arduino CLI から取得した情報が最新状態に保たれます。
  生成処理では `arduino-cli compile --show-properties` の結果などを解析してデバッガー向け設定を自動抽出し、CLI 環境の内容がそのまま反映されます。ファイル更新後はコマンドが Arduino CLI のデバッグビルドタスクを実行し、続けて該当構成でセッションを開始するので、初回から追加操作なしで Arduino が設定した `thb setup` ブレークポイント（`setup()` の先頭）で停止します。
  初回はデバッグオプション付きで全ソースを再コンパイルするため時間がかかります。VS Code から「処理を続行しますか」といったダイアログが表示されることがありますが、そのまま閉じずに待てばコンパイルと書き込みが完了し、自動的にデバッグが開始されます。
3. **構成を常に最新に保つ。** ハードウェアやプロファイルを切り替えたとき、あるいは Arduino CLI やツールチェーンを更新したときは毎回「Arduino CLI: Debug Sketch」を再実行してください。タスクと起動設定を再生成し、自動で次のデバッグセッションを起動します。
4. **生成済みの設定を再利用する。** 直前の実行から設定に変更がなければ、Run and Debug ビューや **F5** からデバッグを開始できます。どちらもコマンドが用意した構成とビルド成果物を再利用します。
5. **必要に応じてカスタマイズする。** 生成された `launch.json` は自由に編集できます（例: `overrideAttachCommands` の追加、semihosting コマンド、別の SVD への差し替えなど）。次回再生成するときも極力マージされます。ツールチェーンを差し替えたらコマンドを再実行し、Arduino CLI から新しいパスを再取得してください。

ヒント:
- デバッガー向け設定は拡張機能が自動生成します。「Arduino CLI: Debug Sketch」を実行するたびに `arduino-cli compile --show-properties` などのメタデータを解析するため、デバッガー引数を手作業で管理する必要はありません。

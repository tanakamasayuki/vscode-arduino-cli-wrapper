...
手早く扱いたいだけなら **Arduino CLI: アセットを埋め込む** が便利です。`assets/` にファイルを置いてコマンドを実行すると（フォルダーがなければ自動作成）、`.ino` と同じ場所に `assets_embed.h` が生成され、各ファイルが `PROGMEM` のバイト配列と長さ定数に展開されます。再生成もコマンド一発で済み、ヘッダーより新しいアセットがあればコンパイル時に警告を出します。

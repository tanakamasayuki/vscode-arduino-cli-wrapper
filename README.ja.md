...
アセット生成には公開済みの [`embed-asset-tool`](https://www.npmjs.com/package/embed-asset-tool) パッケージを使用しています。このパッケージは単独の CLI としても利用でき、拡張機能の手動コマンド、コンパイル前の再生成、診断も同じパッケージ API を利用します。

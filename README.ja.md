...
手早く扱いたいだけなら **Arduino CLI: アセットを埋め込む** が便利です。Arduino CLI エクスプローラーの各プロファイル欄（Upload Data の直前）からコマンドを呼び出し、`assets/` にファイルを置けば、各ファイルを `PROGMEM` のバイト配列と長さ定数に展開した `assets_embed.h` を再生成します。フォルダーが存在しない場合は手動コマンド実行時に自動作成されますが、コンパイル前の自動再生成では `assets/` が無いときにヘッダーを作らず終了するため、意図せず復活することはありません。
